...
class User:
    ...
    def info_full(self, **kwargs) -> dict:
        ...
        if not self.username and not self.user_id:
            raise TypeError(
                "You must provide the username or user_id when creating this class to use this method."
            )
        ...
        if self.username:
            ...
            path = "api/im/multi_user/?{}&{}".format(
                User.parent._add_url_params(), urlencode(query)
            )

            user_props = User.parent.get_data(path, send_tt_params=True, **kwargs)
            return user_props['userMap']
